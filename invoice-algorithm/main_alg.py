...
class main_alg(object):
    def __init__(self):
        ...
        if cfg.algorithm_bool['pretrained_model_eval_dataset2']:
            self.Pretrained_model_Eval_dataset2()

        elif cfg.algorithm_bool['train_dataset1_eval_dataset2']:
            alg = self.Train_dataset1()
            ...
            ticket_list_train = random.sample(ticket_list_train,len(ticket_list_train))
            val_list = ticket_list_train[18000:]
            ticket_list_train = ticket_list_train[:18000]
            amounts = range(100,len(ticket_list_train),100)
            
            accs = []
            f1s = []
            for a in amounts:
                alg = BERT.BERT(classes)
                tickets = ticket_list_train[:a]
                alg.Tickets_to_arrays(tickets)
                alg.Dataset_to_train()
                print("READY TO TRAIN - dataamount: " + str(a))
                alg.Train()

                alg.Tickets_to_arrays(val_list)
                alg.Dataset_to_test()
                ...
                preds = alg.Y_pred
                preds_freq = dict(collections.Counter(preds))
                print("predicted label frequency")
                print(preds_freq)
        
                f1 = f1_score(preds,test_labels,average='macro')
                accuracy = accuracy_score(preds,test_labels)
                print(f"datasize: {len(ticket_list_train)}, accuracy: {accuracy}, f1: {f1}")
                f1s.append(f1)
                accs.append(accuracy)
                f1s = [ '%.5f' % float(elem) for elem in f1s ]
                print(f1s)
               
            self.plotProgress(amounts,f1s,"BERT predicitions - f1 score","f1 score")
            self.plotProgress(amounts,accs,"BERT predicitions - accuracy","accuracy")


            #alg = BERT.BERT(classes)
            #ticket_list_train = ticket_list_train
...
#
            #ticket_list_test = []
            #for i in alg.I_test:
            #    ticket_list_test.append(ticket_list_train[i])
#
            #for i,p in enumerate(alg.Y_pred):
            #    ticket_list_test[i].prediction = p
            #
            #output.Write_tickets(wb_train,ticket_list_test,1)
            #output.CloseWorkbook()
#
            #self.tpr_fpr(test_labels,preds,classes,"BERT")
    ...
    def tpr_fpr(self,y_actual, y_hat, classes,title):
        ...
        for key in classes: 
            ...
            class_score = (key,amount,TP,FP,TPR)
